document.getElementById("navbar").innerHTML=
    '<a href="index.html" class="logo_menu"><img src="img/001-LOGO.png" class="logo"></a>'+
                '<ul class="nav-menu">'+
                    '<li class="nav-item">'+
                        '<a href="index.html" class="nav-link">Início</a>'+
                    '</li>'+
                    '<li class="nav-item">'+
                        '<a href="index.html#comofunciona" class="nav-link">Como funciona</a>'+
                    '</li>'+
                    '<li class="nav-item">'+
                        '<a href="index.html#bases" class="nav-link">Bases</a>'+
                    '</li>' +
                    '<li class="nav-item">' +
                        '<a href="index.html#exemplos" class="nav-link">Exemplos</a>'+
                    '</li>' +
                    '<li class="nav-item">' +
                        '<a href="index.html#informativos" class="nav-link">Informativos</a>'+
                    '</li>' +
                    '<li class="nav-item">' +
                        '<a href="index.html#sobre" class="nav-link">Sobre</a>'+
                    '</li>' +
                    '<li class="nav-item">' +
                        '<a href="faq.html" class="nav-link">FAQ</a>' +
                    '</li>' +
                    '<li class="nav-item">' +
                        '<a href="https://api.whatsapp.com/send/?phone=5565993323756&text=Ol%C3%A1%21+Gostaria+de+ajuda+com+o+MAPADOCAR&app_absent=0"  target="_blank" class="nav-link">Suporte</a>' +
                    '</li>' +
                    '<li class="nav-item">' +
                        '<a href="tutorial.html" class="nav-link">Tutorial</a>' +
                    '</li>' +
                '</ul>' +
                '<a href="https://www.mapadocar.com.br/app/entrar" target="_blank" class="botao_entrar"><button class="botao_primario botao_resp">ENTRAR</button></a>' +

                '<div class="hamburger">' +
                    '<span class="bar"></span>' +
                    '<span class="bar"></span>' +
                    '<span class="bar"></span>' +
                '</div>'